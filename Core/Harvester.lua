...
function H:BuildExport()
    ...
    -- NOTE: this in-game export is a quick snapshot only. It intentionally omits
    -- objectives (o) and the daily/world-quest kind (k), which the canonical
    -- build pipeline (_generator/build-from-harvest.js, run against your flushed
    -- SavedVariables) does emit. Prefer that tool for the shipped QuestRewards.lua;
    -- use this box for a fast look or when Node isn't handy.
    local header = "-- QuestTally export snapshot (titles + rewards only -- NOT a full QuestRewards.lua).\n"
        .. "-- Canonical data is generated by _generator/build-from-harvest.js, which also\n"
        .. "-- bakes objectives and quest kind. This box is a quick first-party snapshot.\n"
        .. "local _, DT = ...\n"
...
local function showCopyBox(text, subtitle)
    if not exportFrame then
        ...
        f.hint:SetText("Press Ctrl+A then Ctrl+C to copy. Quick snapshot only -- for the\nshipped data, run _generator/build-from-harvest.js on your SavedVariables.")
