...
local function effectiveLocation(entry, resolved, baked)
    ...
    -- Sort by the zone's real continent/island where we know it, so a quest's
    -- continent follows its geography rather than the expansion that added it (a
    -- Dragonflight skyriding race in Hellfire Peninsula belongs under Outland, not
    -- Dragon Isles). Falls back to the pre-mapped per-expansion default otherwise.
    -- Events are exempt: they intentionally group under the "Holidays & Events"
    -- bucket (their defContinent), not the geographic zone they happen to occupy.
    local zone = entry.zoneName or entry.defZone
    local label = (DT.Zones and DT.Zones:ZoneLabel(entry)) or zone
    local continent = entry.defContinent
    if entry.type ~= "Event" and DT.ZoneMap then
        continent = DT.ZoneMap:ContinentForZone(label, entry.expansion) or continent
    end
        ...
        and (showSeasonal or not entry.seasonal) then
            ...
            if opts.undiscovered ~= false or status ~= DT.STATUS.UNDISCOVERED then
                ...
                list[#list + 1] = {
                    ...
                    freq          = entry.freq or "daily",  -- "daily" (default) | "weekly"
                    giver         = giver,
                }
            end
        end
